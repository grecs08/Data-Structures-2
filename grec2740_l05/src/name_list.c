...
file description
-------------------------------------
 Author:  Heider AliRick Magnotta
 ID:      99999999
...
void insert_name_list(sNAME_LIST *name_list,    // The linked list of names.
	...
	if (name_list != NULL) {
		...
		if (curr != NULL) {
			while (!found && cont) {
				if (strcmp(curr->name.first_name, node->name.first_name) == 0
						&& strcmp(curr->name.last_name, node->name.last_name)
								== 0) {
					found = 1;
				}
		...
		printf(
				"please pass a valid name_list. either name_list does not exist or is undefined");
	}
}
