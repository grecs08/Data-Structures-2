...
//==============================================
{
	...
	if (child->left == NULL && child->right == NULL) {
		...
		child->height = MAX_HEIGHT(child->left->height, child->right->height)
				+ 1;
	}
	// Return new root.
	return (child);
...
//================================================
{
	...
	if (node->left != NULL && node->right != NULL) {
		//printf("%d", node->left->height - node->right->height);
		...
		if (node->height
				== MAX_HEIGHT(node->left->height, node->right->height) + 1) {
			//printf("b");
			is_valid = is_valid * 1;
		} else {
			//printf("bc");
	...
	if (min_node != NULL && max_node != NULL) {
		if (node->data > min_node->data && node->data < max_node->data) {
			is_valid = is_valid * 1;
		} else {
			is_valid = is_valid * 0;
		}
