...
//==========================================
{
	...
	if ( source->size >= MAX_HEAP_SIZE ) {
	   insert_status = FAILURE;             // Cannot insert because have exceeded declared size of Heap storage;
		...
		source->values[source->size] = value; // insert the new value as a leaf
		heapify_up(source); // bring the newly added element to its correct position
		source->size++; // update the size of the heap
...
{
	int end_idx = source->size - 1; // get the index of the last element in the heap
	int child_idx = end_idx;
	int parent_idx = (child_idx - 1) / 2; // calculate the index of the parent of the last element
	int child_value = source->values[child_idx];
	int parent_value = source->values[parent_idx];
#ifdef DEBUG_LEVEL_01
